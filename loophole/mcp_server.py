...
Claude Code（や他の MCP クライアント）から、対象 Windows のデスクトップセッションに
常駐する loophole をネイティブツールとして使えるようにする。Serena のような
「専用 MCP を足して作業の解像度を上げる」立ち位置。

構成:
    MCP クライアント ──stdio(JSONL)──▶ mcp_server.py（Mac ローカル）
                                          └─ TCP ──▶ loophole（対象 Windows・ssh -L 越し）
...
- 対象 Windows への到達は SSH ポートフォワード（ssh -L 9999:127.0.0.1:9999）。認証は SSH に丸投げ。
...
_CONFIG_TEMPLATE = """\
# loophole の設定 — 対象 Windows のアドレスを書いて保存するだけ。1 回でいい。
# 形式は KEY=value。行頭 '#' はコメント。基本は LOOPHOLE_SSH の 1 行を有効にすれば動く。

# 【ほぼ必須】自動 SSH トンネルの宛先（Windows への SSH ログイン先）
...
mcp = FastMCP(
    "loophole",
    instructions=(
        "loophole は対象 Windows PC を SSH 越しに操作する MCP。初回だけ接続設定が要る。\n"
        "セットアップがまだのとき（loophole_status が configured=false、または各ツールが"
        "『未設定』/接続不可を返す）は、ターミナルを開かせず、チャットでこう進めること:\n"
        "  1) ユーザーに『操作したい Windows PC の IP アドレス』を聞く（例 192.168.1.x）\n"
        "  2) ユーザーに『その PC でのユーザー名（サインイン名）』を聞く\n"
        "  3) loophole_configure(windows_ip, username) を呼ぶ\n"
    ...
    try:
        ...
        if not os.environ.get("LOOPHOLE_SSH"):
            raise _AgentError(
                ...
                "Windows PC の IP アドレスとユーザー名を聞いて loophole_configure を呼んでください。"
            ) from exc
        raise _AgentError(
            f"loophole に接続できません（{HOST}:{PORT}、{exc}）。"
            f"~/.loophole/config の LOOPHOLE_SSH を確認、対象 Windows 側で loophole サーバーが "
...
def loophole_status() -> str:
    ...
    their target Windows PC's IP address and their username on it, then call
    ...
    if not configured:
        return ("configured=false — loophole はまだセットアップされていません。"
                "ユーザーに Windows PC の IP アドレスとユーザー名を聞いて "
...
@mcp.tool()
def loophole_configure(windows_ip: str, username: str,
                       ssh_key: str = "", ssh_opts: str = "") -> str:
    """Set up loophole's connection to the target Windows PC, in chat (no terminal).

    Call this when loophole isn't configured yet. First ask the user, in plain words,
    for just two things: (1) the target Windows PC's IP address, (2) their username on
    that PC. Do NOT ask about SSH keys or proxy/jump-host settings — leave ssh_key and
    ssh_opts empty; loophole figures those out by itself.
    ...
    loophole agent on the Windows side answers. Returns a plain-language status to relay.

    Args:
        windows_ip: the Windows PC's IP address, e.g. "192.168.1.x"
        username: the user's account name on that PC (the SSH login user)
    ...
    """
    windows_ip = (windows_ip or "").strip()
    username = (username or "").strip()
    if not windows_ip or not username:
        raise _AgentError("loophole_configure needs both windows_ip and username "
                          "(ask the user for the PC's IP and their username)")
    target = f"{username}@{windows_ip}"
    ...
    try:
        ...
        return (f"設定とトンネルは OK（接続先 {target}）。ただし loophole agent が応答しません。"
                f"対象 Windows 側で loophole サーバーが対話セッションで動いているか確認してください。")
...
def loophole_shell(command: str, encoding: str = "auto") -> str:
    """Run a one-line shell command on the agent host via cmd.exe /S /C and return its output.

    Args:
        command: the command line, e.g. "echo %USERNAME% & ver"
...
def loophole_screenshot() -> Image:
    ...
    Lets you SEE the remote desktop - something an SSH session-0 shell cannot do.
    Use to verify GUI state (e.g. a dialog or a rendered window).
    """
...
def loophole_gui(argv: list[str]) -> str:
    ...
    visible on screen (impossible from an SSH session-0 shell). Returns immediately.

    Args:
        argv: program and arguments, e.g.
...
def _format_ime(r: dict) -> str:
    ...
    if not r.get("supported"):
        return "foreground window has no IME (classic Win32 windows do; some UWP/Electron don't)"
...
def loophole_ime_get() -> str:
    ...
    Check this before sending keystrokes over RDP/VNC: if the IME is ON (Japanese
    input mode), ASCII you type gets swallowed as phonetic reading and garbles. Turn
    it off with loophole_ime_set(open=False) first, then type.

    Reports whether the window supports IME, whether it's open (on/off), the
    conversion mode (hiragana/katakana/alphanumeric...), and roman vs kana input.
    """
...
def loophole_ime_set(open: bool | None = None, mode: str | None = None,
    ...
    keystrokes over RDP/VNC, so typed ASCII isn't eaten by the Japanese IME. (For
    entering Japanese TEXT, prefer loophole_clipboard_set + paste; this tool sets the
    input MODE, not the text.)
    ...
    Classic Win32 windows (Notepad, browser address bars, most editors) honor this;
    some UWP / Electron apps ignore WM_IME_CONTROL and the call will report an error.
    """
    args: dict = {}
    if open is not None:
        args["open"] = open
...
def loophole_menu(action: str, title: str | None = None,
    ...
    """Enumerate or invoke a classic Win32 window's menu bar (blind - no screenshot).

    This drives GUI menus WITHOUT navigating them by keyboard or mouse: list gives you
    every item's command_id, and invoke fires one directly via PostMessage(WM_COMMAND).
    Ideal for brute-forcing/regression-testing an app's menus deterministically.
    ...
    action="invoke": fire the command command_id on the window. Pass the hwnd that
        "list" returned (avoids title ambiguity). PostMessage is async, so this confirms
        the command was posted, not that it finished - observe the result yourself
        (app log, loophole_window list for new dialogs, or re-list to see a toggle flip).

    Only classic Win32 menu bars are supported (Notepad, many native apps, FileMaker).
    Ribbon (Office) / Electron / UWP report "no menu bar"; fall back to loophole_mouse.
    ...
    if action == "list":
        ...
        if amb:
            return amb
        if not r.get("supported"):
            return (f"window hwnd={r.get('hwnd')} has no classic Win32 menu bar "
                    f"(ribbon / Electron / UWP). Use loophole_mouse there.")
...
def _setup_try_ssh(target: str) -> tuple[bool, str]:
    ...
    if ok and not opts:
        ...
        print("     ・対象の Windows PC の電源が入っていない / スリープしている")
...
def _run_setup() -> None:
    ...
    print("\n=== loophole セットアップ ===")
    print("この PC と、操作したい Windows PC をつなぎます。いくつか質問するので答えてください。")
    ...
    ip = _setup_ask("操作したい Windows PC の IP アドレス（例: 192.168.1.x。"
                    "Windows の 設定→ネットワークとインターネット で確認できます）")
    while not ip:
        ip = _setup_ask("IP アドレスを入力してください（例: 192.168.1.x）")
    user = _setup_ask("その Windows PC での あなたのユーザー名（サインインに使う名前）")
    while not user:
    ...
    print("（対象の Windows PC が寝ているときは、先に起こして loophole を動かしておいてください）")
